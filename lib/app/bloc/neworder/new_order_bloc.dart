...
class NewOrderBloc extends Bloc<NewOrderEvent, NewOrderState> {
  ...
  Stream<NewOrderState> mapEventToState(NewOrderEvent event) async* {
    if (event is NewOrderButtonPressed) {
        yield* _mapLoginToState(event.branchId, event.phoneNumber, event.tableNo);
    }
      ...
      yield* _logoutState();
    } else if (event is NavigatorActionLoginInitial) {
      yield* _logoutState();
    }
  }
  ...
  Stream<NewOrderState> _mapLoginToState(String branchId, String phoneNumber, String tableNo) async* {
    try {
